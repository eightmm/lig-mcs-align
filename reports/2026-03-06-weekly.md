...
All three GIFs below were rendered with the same `100` optimization steps. This set was updated to favor larger query molecules with more substantial MCS overlap instead of tiny molecules that only look “high coverage” because the query is small.

Broader but larger analog:

![Broader overlap acemetacin](../examples/10gs/visualizations/coverage/broader_acemetacin_100steps.gif)

- `Acemetacin`: query coverage `38.5%`, heavy atoms `26`

Mid-overlap analog:

![Mid overlap diclofenac](../examples/10gs/visualizations/coverage/mid_diclofenac_100steps.gif)

- `Diclofenac`: query coverage `52.6%`, heavy atoms `19`

Higher-overlap analog:

![Higher overlap flurbiprofen](../examples/10gs/visualizations/coverage/high_flurbiprofen_100steps.gif)

- `Flurbiprofen`: query coverage `71.4%`, heavy atoms `14`

Why it matters:

- this keeps the optimization budget fixed at `100` steps while shifting the gallery toward more meaningful overlap cases
- it reduces the visual inconsistency that came from mixing different step counts and very small query ligands
- it shows a more useful progression from broader scaffold sharing to higher-overlap analogs under the same reference
