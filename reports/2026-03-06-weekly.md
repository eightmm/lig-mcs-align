...
All GIFs below were rendered with the same `100` optimization steps. The gallery is now split into a `query coverage` view and a `reference coverage` view so both overlap axes are visible in the report.
...
Reference-focused set:

Lower reference coverage:

![Ref low tolmetin](../examples/10gs/visualizations/coverage/ref_low_tolmetin_100steps.gif)

- `Tolmetin`: query `36.8%`, ref `21.2%`, heavy atoms `19`

Mid reference coverage:

![Ref mid fenbufen](../examples/10gs/visualizations/coverage/ref_mid_fenbufen_100steps.gif)

- `Fenbufen`: query `38.1%`, ref `24.2%`, heavy atoms `21`

Higher reference coverage:

![Ref high acemetacin](../examples/10gs/visualizations/coverage/ref_high_acemetacin_100steps.gif)

- `Acemetacin`: query `38.5%`, ref `30.3%`, heavy atoms `26`

Why it matters:

- this keeps the optimization budget fixed at `100` steps while separating the two overlap questions that matter in practice
- the query-focused set shows how much of the query is anchored under a comparable reference core
- the reference-focused set shows how much of the native reference scaffold is reused by the analog
